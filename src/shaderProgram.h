...
public:
    ...
    // Method to validate a shader program. Separate to give users a chance to add uniforms.
    void validate()
    {
  ...
  : ShaderProgram() {
    ...
    void set(const std::string uniformName, float uniformValue) {
        glUniform1f(uniform(uniformName), uniformValue);
    }

    void seti(const std::string uniformName, int uniformValue) {
        glUniform1i(uniform(uniformName), uniformValue);
    }

    void setfv(const std::string uniformName, int n, float* v) {
        glUniform1fv(uniform(uniformName), n, v);
    }

    int addUniform(const std::string uniformName, float uniformValue) {
    ...
    int addUniformfv(const std::string uniformName, int n, float* v) {
        addUniform(uniformName);
        use();
        setfv(uniformName, n, v);
        return uniformMap[uniformName];
    }
